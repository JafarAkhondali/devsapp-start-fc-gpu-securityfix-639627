...
def partition(model_path: str, pp_size: int, dest_pp_size, model_part: int):
    if model_path.endswith("/"):
        model_path = model_path[:-1]

    config = AutoConfig.from_pretrained(model_path, trust_remote_code=True)
    if config.torch_dtype == None or config.torch_dtype == torch.float32:
    ...
    if pp_size == 1:
        ...
        save_file(model.state_dict(), model_path + "/model.safetensors")
        return
    ...
    for id in range(start_pp_rank, end_pp_rank + 1):
        ...
        if id < dest_pp_size and dest_pp_size < pp_size:
            # write dest_num_hidden_layers to model_path/dest_info
            dest_num_hidden_layers = dest_layers[id] - (0 if id == 0 else dest_layers[id-1])
            with open(model_path_+ "/dest_info", "w") as f:
